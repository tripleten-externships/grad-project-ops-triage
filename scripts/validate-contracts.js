...
function validateData(dataPath, schema, schemaName, dataName) {
  ...
  if (Array.isArray(data)) {
    ...
    if (invalidCount === 0) {
      ...
      error(
        `${invalidCount} of ${data.length} items in ${dataName} failed validation`
      );
    }
  } else {
...
function main() {
  ...
  const requestSchemaPath = path.join(
    __dirname,
    '../contracts/schemas/request.schema.json'
  );
  const userSchemaPath = path.join(
    __dirname,
    '../contracts/schemas/user.schema.json'
  );

  // Define data paths
  const requestsDataPath = path.join(
    __dirname,
    '../contracts/mock-data/requests.json'
  );
  const usersDataPath = path.join(
    __dirname,
    '../contracts/mock-data/users.json'
  );
  ...
  const requestSchema = validateSchema(
    requestSchemaPath,
    'request.schema.json'
  );
  ...
  console.log('');
  console.log(
    `${colors.blue}Step 4: Validating integration schemas (optional)...${colors.reset}`
  );
  console.log('');
