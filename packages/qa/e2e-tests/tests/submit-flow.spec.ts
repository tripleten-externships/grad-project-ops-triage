...
test.describe('Submit Request Flow (US-01)', () => {
  ...
  test('TC-01-001: Submit valid request (happy path)', async ({ page }) => {
    ...
    await page.fill(
      ...
      'I forgot my password and cannot log in to my email account'
    );
  ...
  test('TC-01-006: Validation - empty description shows error', async ({
    page,
  }) => {
  ...
  test('TC-01-009: Special characters in title handled correctly', async ({
    page,
  }) => {
  ...
  test('TC-01-017: Double-click submit prevented (no duplicate requests)', async ({
    page,
  }) => {
    await page.fill(
      '[data-testid="request-title"]',
    ...
    await expect(page.locator('[data-testid="success-message"]')).toHaveCount(
      1
    );
  });
