...
export function validatePrediction(prediction: Prediction): Prediction {
  ...
  if (!VALID_CATEGORIES.includes(prediction.category)) {
    ...
    prediction.category_confidence = Math.min(
  ...
  if (!VALID_PRIORITIES.includes(prediction.priority)) {
    ...
    prediction.priority_confidence = Math.min(
...
export function applyGuardrails(prediction: Prediction): GuardrailedResult {
  ...
  const confThreshold = parseFloat(
    process.env.CATEGORY_CONFIDENCE_THRESHOLD || '0.6'
  );
  if (
  ...
  const isUnlikely = unlikelyCombos.some(
    (combo) =>
      combo.category === prediction.category &&
      combo.priority === prediction.priority
  );

  if (isUnlikely) {
    result.needs_manual_review = true;
    result.review_reason =
      result.review_reason || 'Unusual category-priority combination';
  }
